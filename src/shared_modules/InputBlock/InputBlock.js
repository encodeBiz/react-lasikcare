...
const InputBlock = (props) => {
	...
	const { label, type, values, placeholder, handleChange, handleBlur, paramBlur, paramChange } =
		props;
	const inputType =
		type !== "password" ? type : type === "password" && !isPassVisible ? "password" : "text";
		...
		<label className={type === "password" ? "password-input" : ""}>
			...
			{type !== "password" ? (
				...
				<span
					...
					className={isPassVisible ? "show-password" : "hide-password"}
				></span>
			)}
		</label>
	);
